...
export default function ExternalEvaluations() {
    ...
    try {
      const response = await fetch('/api/external-evaluations', {
        ...
        body: JSON.stringify({
          ...
          title: `Evaluación DISC - ${new Date().toLocaleDateString(language === 'es' ? 'es-ES' : 'en-US')}`,
        }),
      });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/external-evaluations/${evaluationToDelete.token}/delete`, {
        method: 'DELETE',
      });
  ...
  const handleResendEmail = async (token: string, recipientName: string) => {
    ...
    try {
      const response = await fetch('/api/external-evaluations/resend', {
        ...
        body: JSON.stringify({ token, type: 'DISC' }),
      });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {t('extEval.disc.title')}
          </h1>
          <p className="text-lg text-gray-600">
            {t('extEval.disc.subtitle')}
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid w-full grid-cols-2 lg:w-[400px] bg-gray-100 p-1 rounded-md">
            <Button
              ...
              className={`flex items-center gap-2 ${activeTab === 'send' ? 'bg-indigo-600 hover:bg-indigo-700' : ''}`}
            >
            ...
            <Button
              ...
              className={`flex items-center gap-2 ${activeTab === 'manage' ? 'bg-indigo-600 hover:bg-indigo-700' : ''}`}
            >
          ...
          {activeTab === 'send' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {/* Información */}
              <div className="space-y-6">
                <Card className="bg-indigo-50 border-indigo-200 border shadow-lg">
                  <CardHeader>
          ...
          {/* Gestionar Evaluaciones Enviadas */}
          {activeTab === 'manage' && (
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Brain className="w-5 h-5 text-indigo-600" />
                  {t('extEval.disc.manageTitle')}
                </CardTitle>
                <CardDescription>
                  {t('extEval.disc.manageDesc')}
                </CardDescription>
              </CardHeader>
              <CardContent>
                          ...
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                              ...
                              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-sm text-gray-500">
                                ...
                                {evaluation.senderName && (
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span className={evaluation.isOwnEvaluation ? '' : 'font-medium text-indigo-600'}>
                            ...
                            <div className="flex flex-wrap gap-2 w-full md:w-auto">
                              {!isExpired && evaluation.status !== 'COMPLETED' && (
                                <>
                                  ...
                                  <Button
                                    ...
                                    className="bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50 hover:border-indigo-300 flex-1 sm:flex-none"
                                  >
                              ...
                              {evaluation.status === 'COMPLETED' && evaluation.result && (
                                <>
                                  <Link href={`/external-evaluation-results/${evaluation.token}`} className="flex-1 sm:flex-none">
