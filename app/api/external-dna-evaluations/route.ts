...
// GET - List all external DNA evaluations for the team
export async function GET(request: NextRequest) {
  try {
    ...
    const evaluations = await prisma.externalDNAEvaluation.findMany({
    ...
    console.error('Error fetching external DNA evaluations:', error);
...
// POST - Create a new external DNA evaluation
export async function POST(request: NextRequest) {
  try {
    ...
    const evaluation = await prisma.externalDNAEvaluation.create({
      data: {
        title: title || `Evaluación DNA-25 para ${recipientName}`,
        description: description || 'Evaluación de Competencias DNA-25',
        recipientName,
        recipientEmail,
    ...
    console.error('Error creating external DNA evaluation:', error);
    return NextResponse.json(
