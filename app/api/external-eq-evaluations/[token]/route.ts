...
import { calculateEQScores } from '@/lib/eq-calculator';
...
export async function POST(
  ...
  try {
    ...
    const responsePromises = responses.map(async (response: { questionId: string; selectedValue: number }) => {
      return prisma.externalEQResponse.upsert({
        where: {
    ...
    const scoringResponses = responses.map((r: { questionId: string; selectedValue: number }) => {
      ...
      return {
        ...
        dimension: question?.dimension || 'SELF_AWARENESS',
    ...
    const scores = calculateEQScores(scoringResponses);
    
    // Save result
    const result = await prisma.externalEQResult.upsert({
      where: { externalEvaluationId: evaluation.id },
      update: {
        eqProfile: scores.eqProfile,
        eqLevel: scores.eqLevel,
        totalEQPercentile: scores.totalEQPercentile,
        selfAwarenessPercentile: scores.selfAwarenessPercentile,
        selfRegulationPercentile: scores.selfRegulationPercentile,
        motivationPercentile: scores.motivationPercentile,
        empathyPercentile: scores.empathyPercentile,
        socialSkillsPercentile: scores.socialSkillsPercentile,
    ...
    await prisma.externalEQEvaluation.update({
    ...
    try {
      await updateCampaignCandidateOnCompletion(token, 'EQ');
    } catch (campaignError) {
    ...
    console.error('Error submitting EQ evaluation:', error);
    return NextResponse.json(
