...
import { Plus, Mail, Send, Users, Dna, CheckCircle, Clock, AlertTriangle, Calendar, Eye, Copy, Trash2, RefreshCw } from 'lucide-react';
...
interface ExternalDNAEvaluation {
...
export default function ExternalDNAEvaluations() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const { t, language } = useLanguage();
  const [evaluations, setEvaluations] = useState<ExternalDNAEvaluation[]>([]);
  ...
  const [evaluationToDelete, setEvaluationToDelete] = useState<ExternalDNAEvaluation | null>(null);
  ...
  const fetchEvaluations = async () => {
    try {
      const response = await fetch('/api/external-dna-evaluations');
  ...
  const copyInvitationLink = async (token: string) => {
    await navigator.clipboard.writeText(`${window.location.origin}/external-dna-evaluation/${token}`);
    setSuccess(t('extEval.success.copied')); setTimeout(() => setSuccess(''), 3000);
  };

  const handleDeleteClick = (evaluation: ExternalDNAEvaluation) => { setEvaluationToDelete(evaluation); setDeleteDialogOpen(true); };

  const handleDeleteConfirm = async () => {
    if (!evaluationToDelete) return; setDeleting(true); setError('');
    try {
      const response = await fetch(`/api/external-dna-evaluations/${evaluationToDelete.token}/delete`, { method: 'DELETE' });
  ...
  const handleResendEmail = async (token: string, recipientName: string) => {
    ...
    try {
      const response = await fetch('/api/external-evaluations/resend', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token, type: 'DNA' }) });
  ...
  const getStatusBadge = (evaluation: ExternalDNAEvaluation) => {
  ...
  if (status === 'loading' || loading) {
    return (<div className="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-50 flex items-center justify-center">
      <div className="text-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-teal-600 mx-auto mb-4"></div><p className="text-gray-600">{t('extEval.loading')}</p></div>
    </div>);
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-6">
          <div className="grid w-full grid-cols-2 lg:w-[400px] bg-gray-100 p-1 rounded-md">
            <Button variant={activeTab === 'send' ? 'default' : 'ghost'} onClick={() => setActiveTab('send')} className={`flex items-center gap-2 ${activeTab === 'send' ? 'bg-teal-600 hover:bg-teal-700' : ''}`}><Send className="w-4 h-4" />{t('extEval.tabs.send')}</Button>
            <Button variant={activeTab === 'manage' ? 'default' : 'ghost'} onClick={() => setActiveTab('manage')} className={`flex items-center gap-2 ${activeTab === 'manage' ? 'bg-teal-600 hover:bg-teal-700' : ''}`}><Users className="w-4 h-4" />{t('extEval.tabs.manage')} ({evaluations.length})</Button>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2">
                <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl">
                  <CardHeader><CardTitle className="flex items-center gap-2"><Mail className="w-5 h-5 text-teal-600" />{t('extEval.dna.formTitle')}</CardTitle><CardDescription>{t('extEval.form.description')}</CardDescription></CardHeader>
                  <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <Button type="submit" disabled={submitting} className="w-full bg-teal-600 hover:bg-teal-700">{submitting ? (<><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>{t('extEval.form.sending')}</>) : (<><Send className="w-4 h-4 mr-2" />{t('extEval.form.send')}</>)}</Button>
                    </form>
                  </CardContent>
                </Card>
              </div>
              <div className="space-y-6">
                <Card className="bg-teal-50 border-teal-200 border shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2 text-teal-800 text-lg">✅ {t('extEval.howItWorks.title')}</CardTitle></CardHeader><CardContent className="space-y-3">{[1,2,3,4].map(i => (<div key={i} className="flex items-start gap-3"><div className="w-6 h-6 bg-teal-600 text-white rounded-full flex items-center justify-center text-sm font-bold">{i}</div><p className="text-teal-700 text-sm">{t(`extEval.howItWorks.step${i}`)}</p></div>))}</CardContent></Card>
          ...
          {activeTab === 'manage' && (
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
              <CardHeader><CardTitle className="flex items-center gap-2"><Dna className="w-5 h-5 text-teal-600" />{t('extEval.dna.manageTitle')}</CardTitle><CardDescription>{t('extEval.dna.manageDesc')}</CardDescription></CardHeader>
              <CardContent>
                {evaluations.length === 0 ? (
                  <div className="text-center py-12"><Dna className="w-16 h-16 text-gray-400 mx-auto mb-4" /><h3 className="text-lg font-medium text-gray-900 mb-2">{t('extEval.manage.empty')}</h3><p className="text-gray-600 mb-6">{t('extEval.manage.emptyDesc')}</p><Button onClick={() => setActiveTab('send')} className="bg-teal-600 hover:bg-teal-700"><Plus className="w-4 h-4 mr-2" />{t('extEval.manage.sendFirst')}</Button></div>
                ) : (
                          ...
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-sm text-gray-500">
                                ...
                                {evaluation.senderName && (<div className="flex items-center gap-1"><Users className="w-4 h-4 text-teal-500" /><span>{t('extEval.manage.sentBy')}: {evaluation.isOwnEvaluation ? t('extEval.manage.you') : evaluation.senderName}</span></div>)}
                            ...
                            <div className="flex flex-wrap gap-2 w-full md:w-auto">
                              {!isExpired && evaluation.status !== 'COMPLETED' && (<><Button size="sm" variant="outline" onClick={() => copyInvitationLink(evaluation.token)} className="bg-white flex-1 sm:flex-none"><Copy className="w-4 h-4 mr-1" />{t('extEval.actions.copyLink')}</Button><Button size="sm" variant="outline" onClick={() => handleResendEmail(evaluation.token, evaluation.recipientName)} disabled={resending === evaluation.token} className="bg-white text-teal-600 border-teal-200 hover:bg-teal-50 flex-1 sm:flex-none">{resending === evaluation.token ? <RefreshCw className="w-4 h-4 mr-1 animate-spin" /> : <Send className="w-4 h-4 mr-1" />}{t('extEval.actions.resend')}</Button></>)}
                              {evaluation.status === 'COMPLETED' && evaluation.result && (<><Link href={`/external-dna-evaluation-results/${evaluation.token}`} className="flex-1 sm:flex-none"><Button size="sm" className="bg-teal-600 hover:bg-teal-700 w-full"><Eye className="w-4 h-4 mr-1" />{t('extEval.actions.viewResults')}</Button></Link><ExternalResultsExportButton type="dna" token={evaluation.token} recipientName={evaluation.recipientName} /></>)}
                              <EvaluationNotesButton evaluationType="DNA" evaluationId={evaluation.token} recipientEmail={evaluation.recipientEmail} recipientName={evaluation.recipientName} />
