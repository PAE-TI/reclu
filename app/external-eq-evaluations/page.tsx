...
interface ExternalEQEvaluation {
  id: string;
  title: string;
  description?: string;
...
export default function ExternalEQEvaluations() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const { t, language } = useLanguage();
  const [evaluations, setEvaluations] = useState<ExternalEQEvaluation[]>([]);
  ...
  const [evaluationToDelete, setEvaluationToDelete] = useState<ExternalEQEvaluation | null>(null);
  ...
  const fetchEvaluations = async () => {
    try {
      const response = await fetch('/api/external-eq-evaluations');
    ...
    try {
      const response = await fetch('/api/external-eq-evaluations', {
        ...
        body: JSON.stringify({
          ...
          title: `Evaluación EQ - ${new Date().toLocaleDateString(language === 'es' ? 'es-ES' : 'en-US')}`,
        }),
      });
  ...
  const copyInvitationLink = async (token: string) => {
    const link = `${window.location.origin}/external-eq-evaluation/${token}`;
    await navigator.clipboard.writeText(link);
    setSuccess(t('extEval.success.copied'));
    setTimeout(() => setSuccess(''), 3000);
  };

  const handleDeleteClick = (evaluation: ExternalEQEvaluation) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/external-eq-evaluations/${evaluationToDelete.token}/delete`, {
        method: 'DELETE',
      });
  ...
  const handleResendEmail = async (token: string, recipientName: string) => {
    ...
    try {
      const response = await fetch('/api/external-evaluations/resend', {
        ...
        body: JSON.stringify({ token, type: 'EQ' }),
      });
  ...
  const getStatusBadge = (evaluation: ExternalEQEvaluation) => {
        ...
        <div className="space-y-6">
          ...
          <div className="grid w-full grid-cols-2 lg:w-[400px] bg-gray-100 p-1 rounded-md">
            <Button
              ...
              className={`flex items-center gap-2 ${activeTab === 'send' ? 'bg-rose-600 hover:bg-rose-700' : ''}`}
            >
            ...
            <Button
              ...
              className={`flex items-center gap-2 ${activeTab === 'manage' ? 'bg-rose-600 hover:bg-rose-700' : ''}`}
            >
          ...
          {activeTab === 'send' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-6">
                <Card className="bg-rose-50 border-rose-200 border shadow-lg">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-start gap-3">
                      ...
                      <p className="text-rose-700 text-sm">{t('extEval.howItWorks.step1')}</p>
                    </div>
                    <div className="flex items-start gap-3">
                      <div className="w-6 h-6 bg-rose-600 text-white rounded-full flex items-center justify-center text-sm font-bold">2</div>
                      <p className="text-rose-700 text-sm">{t('extEval.howItWorks.step2')}</p>
                    </div>
                    <div className="flex items-start gap-3">
                      <div className="w-6 h-6 bg-rose-600 text-white rounded-full flex items-center justify-center text-sm font-bold">3</div>
                      <p className="text-rose-700 text-sm">{t('extEval.howItWorks.step3')}</p>
                    </div>
                    <div className="flex items-start gap-3">
                      <div className="w-6 h-6 bg-rose-600 text-white rounded-full flex items-center justify-center text-sm font-bold">4</div>
                      <p className="text-rose-700 text-sm">{t('extEval.howItWorks.step4')}</p>
                    </div>
                  </CardContent>
                </Card>
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {t('extEval.eq.manageTitle')}
                </CardTitle>
                <CardDescription>
                  {t('extEval.eq.manageDesc')}
                </CardDescription>
              </CardHeader>
              <CardContent>
                          ...
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                              ...
                              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-sm text-gray-500">
                                ...
                                {evaluation.senderName && (
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span className={evaluation.isOwnEvaluation ? '' : 'font-medium text-rose-600'}>
                            ...
                            <div className="flex flex-wrap gap-2 w-full md:w-auto">
                              {!isExpired && evaluation.status !== 'COMPLETED' && (
                                <>
                                  ...
                                  <Button
                                    ...
                                    className="bg-white text-rose-600 border-rose-200 hover:bg-rose-50 hover:border-rose-300 flex-1 sm:flex-none"
                                  >
                              ...
                              {evaluation.status === 'COMPLETED' && evaluation.result && (
                                <>
                                  <Link href={`/external-eq-evaluation-results/${evaluation.token}`} className="flex-1 sm:flex-none">
                                    <Button size="sm" className="bg-rose-600 hover:bg-rose-700 w-full">
