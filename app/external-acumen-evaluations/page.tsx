...
import { Plus, Mail, Send, Users, Sparkles, CheckCircle, Clock, AlertTriangle, Calendar, Eye, Copy, Trash2, RefreshCw } from 'lucide-react';
...
interface ExternalAcumenEvaluation {
...
export default function ExternalAcumenEvaluations() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const { t, language } = useLanguage();
  const [evaluations, setEvaluations] = useState<ExternalAcumenEvaluation[]>([]);
  ...
  const [evaluationToDelete, setEvaluationToDelete] = useState<ExternalAcumenEvaluation | null>(null);
  ...
  const fetchEvaluations = async () => {
    try {
      const response = await fetch('/api/external-acumen-evaluations');
  ...
  const copyInvitationLink = async (token: string) => {
    await navigator.clipboard.writeText(`${window.location.origin}/external-acumen-evaluation/${token}`);
    setSuccess(t('extEval.success.copied')); setTimeout(() => setSuccess(''), 3000);
  };

  const handleDeleteClick = (evaluation: ExternalAcumenEvaluation) => { setEvaluationToDelete(evaluation); setDeleteDialogOpen(true); };

  const handleDeleteConfirm = async () => {
    if (!evaluationToDelete) return; setDeleting(true); setError('');
    try {
      const response = await fetch(`/api/external-acumen-evaluations/${evaluationToDelete.token}/delete`, { method: 'DELETE' });
  ...
  const handleResendEmail = async (token: string, recipientName: string) => {
    ...
    try {
      const response = await fetch('/api/external-evaluations/resend', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token, type: 'ACUMEN' }) });
  ...
  const getStatusBadge = (evaluation: ExternalAcumenEvaluation) => {
  ...
  if (status === 'loading' || loading) {
    return (<div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 flex items-center justify-center">
      <div className="text-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-600 mx-auto mb-4"></div><p className="text-gray-600">{t('extEval.loading')}</p></div>
    </div>);
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-6">
          <div className="grid w-full grid-cols-2 lg:w-[400px] bg-gray-100 p-1 rounded-md">
            <Button variant={activeTab === 'send' ? 'default' : 'ghost'} onClick={() => setActiveTab('send')} className={`flex items-center gap-2 ${activeTab === 'send' ? 'bg-amber-600 hover:bg-amber-700' : ''}`}><Send className="w-4 h-4" />{t('extEval.tabs.send')}</Button>
            <Button variant={activeTab === 'manage' ? 'default' : 'ghost'} onClick={() => setActiveTab('manage')} className={`flex items-center gap-2 ${activeTab === 'manage' ? 'bg-amber-600 hover:bg-amber-700' : ''}`}><Users className="w-4 h-4" />{t('extEval.tabs.manage')} ({evaluations.length})</Button>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2">
                <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl">
                  <CardHeader><CardTitle className="flex items-center gap-2"><Mail className="w-5 h-5 text-amber-600" />{t('extEval.acumen.formTitle')}</CardTitle><CardDescription>{t('extEval.form.description')}</CardDescription></CardHeader>
                  <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <Button type="submit" disabled={submitting} className="w-full bg-amber-600 hover:bg-amber-700">{submitting ? (<><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>{t('extEval.form.sending')}</>) : (<><Send className="w-4 h-4 mr-2" />{t('extEval.form.send')}</>)}</Button>
                    </form>
                  </CardContent>
                </Card>
              </div>
              <div className="space-y-6">
                <Card className="bg-amber-50 border-amber-200 border shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2 text-amber-800 text-lg">✅ {t('extEval.howItWorks.title')}</CardTitle></CardHeader><CardContent className="space-y-3">{[1,2,3,4].map(i => (<div key={i} className="flex items-start gap-3"><div className="w-6 h-6 bg-amber-600 text-white rounded-full flex items-center justify-center text-sm font-bold">{i}</div><p className="text-amber-700 text-sm">{t(`extEval.howItWorks.step${i}`)}</p></div>))}</CardContent></Card>
                <Card className="bg-orange-50 border-orange-200 border shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2 text-orange-800 text-lg">⚠️ {t('extEval.important.title')}</CardTitle></CardHeader><CardContent><ul className="text-orange-700 text-sm space-y-2"><li>• {t('extEval.important.expiry')}</li><li>• {t('extEval.important.singleUse')}</li><li>• {t('extEval.important.noAccount')}</li><li>• {t('extEval.important.canDelete')}</li></ul></CardContent></Card>
              </div>
            </div>
          )}
          ...
          {activeTab === 'manage' && (
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
              <CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-amber-600" />{t('extEval.acumen.manageTitle')}</CardTitle><CardDescription>{t('extEval.acumen.manageDesc')}</CardDescription></CardHeader>
              <CardContent>
                {evaluations.length === 0 ? (
                  <div className="text-center py-12"><Sparkles className="w-16 h-16 text-gray-400 mx-auto mb-4" /><h3 className="text-lg font-medium text-gray-900 mb-2">{t('extEval.manage.empty')}</h3><p className="text-gray-600 mb-6">{t('extEval.manage.emptyDesc')}</p><Button onClick={() => setActiveTab('send')} className="bg-amber-600 hover:bg-amber-700"><Plus className="w-4 h-4 mr-2" />{t('extEval.manage.sendFirst')}</Button></div>
                ) : (
                          ...
                          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-sm text-gray-500">
                                ...
                                {evaluation.senderName && (<div className="flex items-center gap-1"><Users className="w-4 h-4 text-amber-500" /><span>{t('extEval.manage.sentBy')}: {evaluation.isOwnEvaluation ? t('extEval.manage.you') : evaluation.senderName}</span></div>)}
                            ...
                            <div className="flex flex-wrap gap-2 w-full md:w-auto">
                              {!isExpired && evaluation.status !== 'COMPLETED' && (<><Button size="sm" variant="outline" onClick={() => copyInvitationLink(evaluation.token)} className="bg-white flex-1 sm:flex-none"><Copy className="w-4 h-4 mr-1" />{t('extEval.actions.copyLink')}</Button><Button size="sm" variant="outline" onClick={() => handleResendEmail(evaluation.token, evaluation.recipientName)} disabled={resending === evaluation.token} className="bg-white text-amber-600 border-amber-200 hover:bg-amber-50 flex-1 sm:flex-none">{resending === evaluation.token ? <RefreshCw className="w-4 h-4 mr-1 animate-spin" /> : <Send className="w-4 h-4 mr-1" />}{t('extEval.actions.resend')}</Button></>)}
                              {evaluation.status === 'COMPLETED' && evaluation.result && (<><Link href={`/external-acumen-evaluation-results/${evaluation.token}`} className="flex-1 sm:flex-none"><Button size="sm" className="bg-amber-600 hover:bg-amber-700 w-full"><Eye className="w-4 h-4 mr-1" />{t('extEval.actions.viewResults')}</Button></Link><ExternalResultsExportButton type="acumen" token={evaluation.token} recipientName={evaluation.recipientName} /></>)}
                              <EvaluationNotesButton evaluationType="ACUMEN" evaluationId={evaluation.token} recipientEmail={evaluation.recipientEmail} recipientName={evaluation.recipientName} />
